...
Here, new keyword is used to allocate “new” memory address “dynamically” in the heap and return that address. Since the right hand side of the above statement returns an address, we need to assign it to a pointer as done above. If this statement is present in a function, then every time a new address location is returned.
*/
